...
test.describe('Use Case 2: Form with Upload Flow - UI Automation', () => {
  ...
  test('Should validate file upload status and form behavior', async ({ page }) => {
    const loginPage = new LoginPage(page);
    const dashboardPage = new DashboardPage(page);
    const formPage = new FormPage(page);
    ...
    await dashboardPage.clickCreateDropdown();
    await dashboardPage.selectForm();
    await formPage.createForm(`FileValidation_${Date.now()}`, 'Testing file upload validation');
    
    await formPage.dragAndDropFileUpload();
    
    // Validate before upload
    await test.step('Validate no file selected initially', async () => {
      const fileInputValue = await formPage.fileInput.inputValue();
      expect(fileInputValue).toBe('');
      console.log('✓ No file selected validation passed');
    });
    ...
    await test.step('Validate file is selected', async () => {
      const fileInputValue = await formPage.fileInput.inputValue();
      expect(fileInputValue).toContain('sample-upload.txt');
      console.log('✓ File selection validation passed');
    });
  });
});
